...
However, using macros comes with a cost. 
Compilation time might be slightly increased and it seems that macros will change in Scala 3. 
It could mean you will have to re-write some of the code. 
That was a bit offtopic. Let's go back to our scenario. 
...
ZIO provides different expectations that you can use to build your assertions: `isEmpty`, `equalTo`, `isLessThen`, `contains` and many others.
...
```
Here we can see one of the service method definitions. 
...
Later, parts of this project were moved into ZIO core and ZIO test. 
...
val gitHubClientMocks = GitHubClientMock.Releases(equalTo(name), value(releases))
```
As we don't want to call a real GitHub service we tell the mock to return us list of expected releases.
...
Having no macros might benefit you in the long-term, when you will move your code to Scala 3.
Also, as we have seen not all IDEs support macro generated code (at the moment of writing).
